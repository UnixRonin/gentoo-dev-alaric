...
IUSE="bindist +jit +minimal pgo pulseaudio selinux system-cairo system-icu system-jpeg system-sqlite test webm"
...
src_configure() {
	...
	# (Well, actually, no, we're not supposed to be using it.)
...
src_install() {
	...
	# 'make install' doesn't pick up bundled add-ons like the PaleMoon statusbar, which
	# is rather impolite of it.  We use this rather inelegant hack to work around that:
        pushd ${S}
	emake -f client.mk package
	cd "${D}"/usr/lib64/
        ln -s ${P} palemoon
	tar xvjf ${S}/${obj_dir}/dist/*.tar.bz2 -K palemoon/distribution --skip-old-files
        rm palemoon
	popd
}
